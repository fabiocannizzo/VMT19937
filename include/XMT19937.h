...
template <size_t RegisterBitLenHw>
class XMT19937Base
{
...
public:
    ...
    static constexpr size_t s_nStates = 1;
...
private:
    ...
    struct RefillCst
    {
        using XVI = SimdRegister<RegisterBitLen, RegisterBitLenHw>;
    ...
    alignas(64) inline static const TemperCst<SimdRegister<std::min<size_t>(s_regLenBitsHw * 4, s_n32InRndCache * 32), s_regLenBitsHw>> s_temperCst{};
    ...
    static FORCE_INLINE void temperRefillBlock_(const uint32_t * __restrict st, uint32_t * __restrict dst)
    {
        ...
        for (size_t i = 0; i < nIterations; ++i) {
            XVmax tmp = temper(XVmax(st), s_temperCst);
    ...
    void NO_INLINE refill()
    {
        ...
        stCur = advanceLoop(n1, stCur, x0, 1, M, s_refillMasks, std::make_integer_sequence<int, nUnroll>{});
        if constexpr (r1 > 0)
            stCur = advanceLoop(1, stCur, x0, 1, M, s_refillMasks, std::make_integer_sequence<int, (int)r1>{});

        // unroll second part of the loop (M-1) iterations
        constexpr size_t n2 = (M - 1) / nUnroll;
        constexpr size_t r2 = (M - 1) % nUnroll;
        stCur = advanceLoop(n2, stCur, x0, 1, M - N, s_refillMasks, std::make_integer_sequence<int, nUnroll>{});
        if constexpr (r2 > 0)
            stCur = advanceLoop(1, stCur, x0, 1, M - N, s_refillMasks, std::make_integer_sequence<int, (int)r2>{});

        // last iteration
        advanceLoop(1, stCur, x0, 1 - N, M - N, s_refillMasks, std::make_integer_sequence<int, 1>{});
