...
\begin{abstract}
Many simulation applications require the generation of long sequences of pseudo-random numbers. Linear recurrences modulo 2 are commonly used as the fundamental building block for constructing pseudo-random number generators with extended periods and excellent statistical properties. These generators consist of a lengthy binary state vector that evolves iteratively through linear recurrences. One widely accepted pseudo-random generator in this category is the Mersenne twister 19937 (MT19937), proposed by Matsumoto and Nishimura \cite{mt19937}, which has been implemented in numerous software libraries and numerical packages. The MT19937's popularity stems from its favorable distribution properties and the simplicity and speed of its algorithm. The linear recurrence responsible for evolving the binary state vector can be implemented using simple bitwise operations. However, this recurrence does not fully utilize the potential for parallelization through SIMD\footnote{Single Instructions Multiple Data} instructions available on modern hardware, limiting opportunities for high-throughput parallelization.
This paper introduces a new SIMD-friendly random number generator, which maintains the same statistical properties and period as the MT19937. It achieves this by combining multiple MT19937 instances de-phased via jump-ahead transformations, and polling each instance in a round-robin order. By evolving all states vectors simultaneously, the new generator fully exploit SIMD hardware.
Comprehensive test results demonstrate that the throughput of the new generator scales roughly linearly with SIMD registers width, demonstrating substantial speedup on modern CPUs.
\end{abstract}
...
In other words, sequence $\boldsymbol{S}$ is obtained by polling each sub-sequence $Z_t$ in round-robin fashion.
If we interpret each sub-sequence as being generated by a distinct MT19937 instance, initialized at states corresponding to jump-ahead transformations by $J$ steps, the combined $\boldsymbol{S}$ preserves the same overall period and statistical properties as the original MT19937 sequence, but its state evolution can be fully vectorized over $M$ parallel instances.
...
Although VMT19937 with $M$=1 is identical to the original MT19937 from an algorithm point of view, we observe that the implementation of VMT19937 is generally faster.
This is because in VMT19937 the tempering transformation \eqref{eq:tempering} is always vectorized with the best instruction set available, regardless of $M$.
Furthermore, VMT19937 is written as a header file, which allows the compiler to produce more optimized code via inlining.
To ensure a fair comparison that measures solely the improvement associated with the new algorithm, we will use row 3 (VMT19937 with $M$=1) as the baseline for comparison.

In rows 4, 5, and 6, we observe the computation time of VMT19937 with $M$=4 when querying PRNs individually, in blocks of 16 (the size of a cache line), and in blocks of 2496 (the size of the entire state vector).
We notice a significant performance improvement as the query block size increases from 1 to 16, and only a marginal additional improvement when the query block size grows from 16 to 2496.
The reason for this improvement is explained in section \ref{seq:blocks}. Similar patterns are observed in rows 7 to 9 and rows 10 to 12 for generators VMT19937 with $M$=8 and $M$=16, respectively.
