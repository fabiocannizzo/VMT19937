...
This paper introduces a new SIMD-friendly random number generator, which maintains the same statistical properties and period as the MT19937. It achieves this by combining the random streams of multiple MT19937 instances, each with state vectors de-phased via jump-ahead transformations, and polling each instance in a round-robin fashion. By evolving their vector states simultaneously, the new generator achieves perfect vectorization, fully leveraging SIMD hardware capabilities. Comprehensive test results demonstrate that this technique enables the generator throughput to scale approximately linearly with the width of the SIMD registers used. This approach provides significant speed improvements, especially on modern CPUs equipped with larger SIMD registers, and allows for efficient generation of random numbers for various simulation applications.
\end{abstract}
