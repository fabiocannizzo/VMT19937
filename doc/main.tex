...
\begin{frontmatter}
\title{A SIMD friendly variation of Mersenne twister 19937}
...
%These entail of a long binary state vector which evolves iteratively via linear transformations.
%A widely adopted pseudo-random generator belonging to this family is the Mersenne twister 19937 (MT19937) proposed by Matsumoto and Nishimura \cite{mt19937}, which is implemented in several software libraries and numerical packages.
%In addition to having good distribution properties, a feature which determined the MT19937 success is the simplicity and speed of its algorithm. The linear transformation which evolves the binary state vector can be simplified to a very small set of elementary bit manipulations. Such transformation however is not amenable to take advantage of SIMD\footnote{Single Instructions Multiple Data} instructions available on modern hardware to achieve further speed up.
%This paper proposes a technique to re-engineer the MT19937 to fully exploit SIMD hardware. The idea is to interleave the state vectors of multiple MT19937 generators dephased via jump-ahead, evolve their states simultaneously via SIMD operations, then generate random numbers by polling them one by one in round robin fashion. Test results demonstrate that the technique allows to achieve a generator throughput which scales almost linearly with the width of the SIMD registers used.
Many simulation applications require the generation of long sequences of pseudo-random numbers. Linear recurrences modulo 2 are widely used as the basic building block for constructing pseudo-random number generators with extended periods and good statistical properties. These generators consist of a lengthy binary state vector that evolves iteratively through linear transformations. Among the well-accepted pseudo-random generators within this category is the Mersenne twister 19937 (MT19937), proposed by Matsumoto and Nishimura \cite{mt19937}, which finds implementation in several software libraries and numerical packages. Aside from its favorable distribution properties, the MT19937's success lies in the simplicity and speed of its algorithm. The linear transformation responsible for evolving the binary state vector can be simplified to a concise set of elementary bit manipulations. However, this transformation does not readily exploit the potential for parallelization via SIMD\footnote{Single Instructions Multiple Data} instructions available on modern hardware to achieve further speed enhancements. This paper presents a general technique to re-engineer the MT19937 to fully leverage SIMD hardware capabilities. The proposed approach interleaves the state vectors of multiple MT19937 generators, dephased via jump-ahead, allowing their states to evolve simultaneously through SIMD operations. Subsequently, random numbers are generated by polling them one by one in a round-robin fashion. Test results demonstrate that this technique enables achieving a generator throughput that scales nearly linearly with the width of the SIMD registers used.
\end{abstract}
...
This paper introduces a technique to re-engineer the MT19937 fully to leverage SIMD hardware and achieve optimal vectorization. The proposed approach is entirely generic, requiring no customization of the algorithm or recalibration of its parameters for different register lengths. Test results demonstrate the effectiveness of the proposed algorithm, whose throughput scales well with the size of available SIMD registers.
...
\label{eq:stepnomod}
\begin{aligned}
x_{k} &= x_{k+m} ~\XOR~ \left(x_{k}~\AND~h ~\OR~ x_{k+1}~\AND~l\right) \star A  \quad &&k = 0, \dots, n-m-1\\
x_{k} &= x_{k+m-n} ~\XOR~ \left(x_{k}~\AND~h ~\OR~ x_{k+1}~\AND~l\right) \star A \quad &&k = n-m, \dots, n-2 \\
x_{k} &= x_{k+m-n} ~\XOR~ \left(x_{k}~\AND~h ~\OR~ x_{0}~\AND~l\right) \star A  \quad &&k = n-1
\end{aligned}
\end{equation}
...
\cline{2-10}
 ...
 & MS-MT19937   &  16   & 32  & 9984 & 1    & AVX512  & n.a.   & n.a. & 5.79 \\
 \hline \multicolumn{1}{|c|}{11}
 & MS-MT19937   &  16 & 32    & 9984 & 16   & AVX512  & n.a.   & n.a. & 1.58 \\
 \hline \multicolumn{1}{|c|}{12}
 & MS-MT19937   &  16  & 32    & 9984 & 9984 & AVX512  & n.a.   & n.a. & 1.27 \\
 \hline
\end{tabular}
...
\section{Conclusion}
The new random generator MS-MT19937 has been introduced, which utilizes multiple independent random streams generated by the traditional MT19937. These streams are combined with de-phased state vectors through a jump ahead initialization technique.

Comprehensive test results demonstrate that perfect vectorization is achieved, resulting in a throughput that scales approximately linearly with the length of the available SIMD registers when random numbers are queried in blocks, thus allows for faster generation of random numbers, on modern CPUs with larger SIMD registers, compared to the original MT19937 algorithm.

Importantly, this technique does not require re-calibration of the parameters of the underlying MT19937 generator. Additionally, it has the potential to be applied to other linear congruential generators, provided an effective jump ahead methodology exists.

Moreover, while the implementation has been illustrated with SIMD instructions, the generality of this technique suggests its applicability to other vectorial processing systems, such as GPUs or FPGAs. This versatility opens up possibilities for optimizing random number generation across various hardware platforms.
...
\bibitem{mt19937} 1998, M. Matsumoto, T. Nishimura, "Mersenne Twister: A 623-dimensionally equidistributed uniform pseudorandom number generator", ACM Trans. on Modeling and Computer 
...
Simulation Vol. 8, No. 1, January pp.3-30 (1998), DOI:10.1145/272991.272995
...
\bibitem{moneta} 2020, S. Y. Jun, P. Canal, J. Apostolakis, A. Gheata, L. Moneta. "Vectorization of random number generation and reproducibility of concurrent particle transport simulation", Journal of Physics: Conference Series, IOP Publishing, DOI: 10.1088/1742-6596/1525/1/012054
